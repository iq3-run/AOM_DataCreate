...
namespace AOM_DataCreate.HtmlParser.KuroWiki {
    internal class OperatorList : HtmlParser {
        ...
        static Regex kurowiki_list_regex = new Regex(@"<a\s+.*?href=""(.+)"".*?>☆(\d)\s+(.+)</a>");
        static Regex kurowiki_chara_name_regex = new Regex(@"(.+?)\s*(?:\(|（)(.+)(?:\)|）)\s*\[(.+)\]");
        static Regex sub_class_regex = new Regex(@"\[(.+)\](?:\s|&\w+;)*(.+)");
        static Regex material_regex = new Regex(@"(.+?)\s*(?:x|×)\s*(\d+)");
        ...
        public List<COperator> getOperators(Dictionary<string, int> material_dic, List<string> paradox_list) {
            ...
            foreach(Match match in kurowiki_list_regex.Matches(HttpUtility.HtmlDecode(Source))) {
                ...
                Task<string?> kurowiki_chara = HttpClientWrapper.GetPage(new Uri(match.Groups[1].Value.Trim()));
                kurowiki_chara.Wait();
                string? kurowiki_chara_str = kurowiki_chara.Result;
                if(kurowiki_chara_str == null) {
                    continue;
                }
                string kurowiki_chara_str2 = HttpUtility.HtmlDecode(HtmlParser.TagRegex.Replace(kurowiki_chara_str, ""));
                StringReader sr = new StringReader(kurowiki_chara_str2);
                ...
                while((line = sr.ReadLine()) != null) {
                    ...
                    if(line.StartsWith("ページ名：" + match.Groups[3].Value.Trim())) {
                        do_check = true;
                        continue;
                    }
                    ...
                    if(do_check) {
                        if(opr.Name.Japanese.Length  == 0 && kurowiki_chara_name_regex.IsMatch(line)) {
                            ...
                            COperator? exists_op = operators.Find(o => o.Name.Japanese.Equals(chara_name[1].Value.Trim()));
                            if(exists_op != null) {
                                break;
                                exists_op.Alternative.Add(opr);
                                opr.Parent = exists_op;
                            }
        ...
        public List<COperator> margeOperator(List<COperator> base_operators, Dictionary<string, int> material_dic, List<string> paradox_list) {
            ...
            List<COperator> new_list = new List<COperator>();
